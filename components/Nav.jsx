...
export default function Nav() {
  ...
  const getModalContent = (path) => {
    ...
    */
      ...
      return {
        ...
        tips: [
          ...
          "① 펑션 콜링\n" +
            ...
            "- 더 빠르고, 더 정확하고, 더 신뢰 가능한 에이전트가 된다. (환각↓, 비용/지연↓)\n" +
    ...
    // gemini 는 비전이라고 부르는 것은 없고 멀티모달이라 텍스트+이미지가 가능함.
    if (path === "/vision") {
      return {
        title: "이미지 이해하기",
        description:
          "이미지를 입력해서 Gemini 장면과 텍스트를 어떻게 읽는지 확인하는 실습을 진행합니다.",
        tips: [
          // ① 6장의 목표
          ...
          // ② Gemini 비전 동작 방식
          "② Gemini 멀티모달 : 이미지 + 텍스트를 함께 이해하는 방식\n" +
            "- Gemini는 기본적으로 멀티모달을 지원하여 텍스트와 이미지(사진, 스크린샷, 다이어그램 등)를 함께 입력으로 받습니다.\n" +
    ...
    if (path === "/vision-quiz") {
      return {
        title: "이미지 기반 퀴즈 만들기",
        description:
          "이미지를 분석해서 자동으로 퀴즈를 만들어 보는 실습을 진행합니다.",
        tips: [
          // ① 이미지 → 퀴즈 흐름
          "① 이미지에서 퀴즈로 이어지는 흐름\n" +
            "- 이미지를 입력하면 Gemini가 장면, 텍스트, 상황을 분석합니다.\n" +
